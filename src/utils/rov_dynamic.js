...
class ROV extends DynamicParameters {
 ...
 constructor() {
   ...
   this.state = np.array([0, 0, 0, pi * 0.0, pi * 0.5, pi * 0.0, 0, 0, 0, 0, 0, 0], {dtype:'float32'});
 ...
 dxdt(state) {
   //...计算状态微分
    ...
    const C = np.array([[0, 0, 0, 0, (m-this.Zdw*K)*w, -(m-this.Ydv*K)*v],
                      [0, 0, 0, -(m-this.Zdw*K)*w, 0, (m-this.Xdu*K)*u],
                      [0, 0, 0, (m-this.Ydv*K)*v, -(m-this.Xdu*K)*u, 0],
                      [0, (m-this.Zdw*K)*w, -(m-this.Ydv*K)*v, 0, (Iz-this.Ndr*K)*r, -(Iy-this.Mdq*K)*q],
                      [-(m-this.Zdw*K)*w, 0, (m-this.Xdu*K)*u, -(Iz-this.Ndr*K)*r, 0, (Ix-this.Kdp*K)*p],
                      [(m-this.Ydv*K)*v, -(m-this.Xdu*K)*u, 0, (Iy-this.Mdq*K)*q, -(Ix-this.Kdp*K)*p, 0]
                      ]);

    const G = np.array([[(W - B) * s5],
                      [-(W - B) * c5 * s4],
                      [-(W - B) * c5 * c4],
    ...
    const D = np.array([[this.Xu+this.Xuabsu*abs(u), 0, 0, 0, 0, 0],
    ...
    const T = np.array([[c6*c5, -s6*c4+c6*s5*s4, s6*s4+c6*c4*s5, 0, 0, 0],
 ...
 ode45(h) {
    ...
    const k2 = h * this.dxdt(
      np.add(this.state, np.multiply(k1, 0.5))  
    );

    const k3 = h * this.dxdt(
      np.add(this.state, np.multiply(k2, 0.5))
    );
    
    const k4 = h * this.dxdt(
      np.add(this.state, k3)
    );

    this.state = np.add(
      np.multiply(np.add(np.add(k1, np.multiply(k2, 2)), np.multiply(k3, 2)), 1/6),
      np.multiply(k4, 1/6)
    );
